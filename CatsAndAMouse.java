...
public class CatsAndAMouse {
    ...
    static String catAndMouse(int x, int y, int z) {
    	String winner;
    	//if the distance between x and z is less than the distance between y and z, then Cat A wins
    	if(Math.abs(z - x) < Math.abs(z - y)) {
    		winner = "Cat A";
    	//if the distance between x and z is greater than the distance between y and z, then Cat B wins	
    	}else if(Math.abs(z - x) > Math.abs(z - y)) {
    		winner = "Cat B";
    	//if x and y are the same distance from z, the mouse gets away
    	}else {
    		winner = "Mouse C";
    	}
    	return winner;
